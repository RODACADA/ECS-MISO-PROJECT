...
class MenuScene(Scene):
    def do_create(self):
        ...
        set_background = ServiceLocator.setting_service.get("assets/cfg/background.json")
        prefab_creator.create_background(self.ecs_world, set_background, self.screen)
        self.text_entities = prefab_creator_interface.create_menus(self.ecs_world)
        ...
        self.blinking_text = prefab_creator_interface.create_blinking_text(self.ecs_world, 
                                                                        "PRESS Z TO START", 
                                                                        prefab_creator_interface.TextAlignment.CENTER
                                                                        ,0.4)

        # Posiciones finales para cada elemento
        self.final_positions = {
            "up": pygame.Vector2(32, 18),
            "high": pygame.Vector2(152, 18),
            "score": pygame.Vector2(70, 28),
            "record": pygame.Vector2(145, 28),
            "logo": pygame.Vector2(55, 60),
            "blinking_text": pygame.Vector2(128, 160),  # Posición final para el texto parpadeante
        }
    ...
    def do_update(self, delta_time):
        ...
        for entity_name, entity_id in {**self.text_entities, "logo": self.logo, "blinking_text": self.blinking_text}.items():
            entity_transform = self.ecs_world.component_for_entity(entity_id, CTransform)
