...
class MenuScene(Scene):
    ...
    def do_create(self):
        ...
        self.start = create_text(self.ecs_world, "PRESS Z TO START GAME", 9, 
                    pygame.Color(255, 255, 0), pygame.Vector2(128, 240), TextAlignment.CENTER)
        self.controls = create_text(self.ecs_world, "Arrows to MOVE - P to PAUSE", 7, 
                    pygame.Color(150, 150, 255), pygame.Vector2(128, 267), TextAlignment.CENTER)
    ...
    def do_update(self, delta_time):
        for text_entity in [self.title, self.start, self.controls]:
